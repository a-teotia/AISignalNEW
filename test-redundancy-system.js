...
const centralized_data_provider_1 = require("./src/lib/centralized-data-provider");
async function testRedundancySystem() {
    ...
    for (const symbol of symbols) {
        ...
        try {
            ...
            const primaryData = await centralized_data_provider_1.CentralizedDataProvider.getComprehensiveData(symbol);
            ...
            centralized_data_provider_1.CentralizedDataProvider.addSecondaryDataSource(symbol, secondarySource1);
            centralized_data_provider_1.CentralizedDataProvider.addSecondaryDataSource(symbol, secondarySource2);
            // 3. Get data again to trigger verification
            console.log('  🔍 Fetching data with verification...');
            const verifiedData = await centralized_data_provider_1.CentralizedDataProvider.getComprehensiveData(symbol);
            ...
            const report = centralized_data_provider_1.CentralizedDataProvider.getVerificationReport(symbol);
            ...
            const consistencyScore = centralized_data_provider_1.CentralizedDataProvider.getDataConsistencyScore(symbol);
...
async function testConflictScenarios(symbol) {
    ...
    centralized_data_provider_1.CentralizedDataProvider.addSecondaryDataSource(symbol, conflictingSource);
    // Get data again to see conflict resolution
    const dataWithConflict = await centralized_data_provider_1.CentralizedDataProvider.getComprehensiveData(symbol);
...
async function testSystemReliability() {
    ...
    for (const testCase of testCases) {
        ...
        try {
            const data = await centralized_data_provider_1.CentralizedDataProvider.getComprehensiveData(testCase.symbol);
...
async function testPerformanceMetrics() {
    ...
    for (const symbol of symbols) {
        for (let i = 0; i < 3; i++) { // 3 requests per symbol
            ...
            try {
                const data = await centralized_data_provider_1.CentralizedDataProvider.getComprehensiveData(symbol);
